...
import os
from yael import Publication, DublinCore, OPFMeta3
...
class LegacyTests(unittest.TestCase):
    ...
    def test_to_translate(self):
        for metadatum in self.publication.container.default_rendition.pac_document. \
                metadata.metadata_by_tag(DublinCore.NS_SUBJECT):
            print(metadatum)

        metadata = self.publication.container.default_rendition \
            .pac_document.metadata.metadata_by_property(OPFMeta3.V_MEDIA_ACTIVE_CLASS)
        if len(metadata) > 0:
            print(metadata[0].v_text)

        for metadatum in self.publication.container.default_rendition \
                .pac_document.metadata.metadata_by_property(OPFMeta3.V_MEDIA_NARRATOR):
            print(metadatum.v_text)

        for metadatum in p.container.default_rendition \
                .pac_document.metadata.metadata_by_property(OPFMeta3.V_MEDIA_DURATION):
            if metadatum.v_refines is None:
